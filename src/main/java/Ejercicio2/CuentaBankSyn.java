...
public class CuentaBankSyn implements CuentaBankInterface{
    ...
    public synchronized boolean retirar(double cantidad) {
        ...
        if (cantidad <= cuentaBank.getSaldo()) {
            ...
            cuentaBank.getHistorial().add(LocalTime.now() + " RETIRO FALLIDO: " + String.format("%.2f€", cantidad) + " | Fondos insuficientes | SALDO: " + String.format("%.2f€", cuentaBank.getSaldo()));
        }
        return retirado;
    }
    ...
    public synchronized void ingresar(double cantidad) {
        cuentaBank.setSaldo(cuentaBank.getSaldo() + cantidad);
        cuentaBank.getHistorial().add(LocalTime.now() + " INGRESO: +" + String.format("%.2f€", cantidad) + " | SALDO: " + String.format("%.2f€", cuentaBank.getSaldo()));
    }

    @Override
    public synchronized double consultarSaldo() {
        cuentaBank.getHistorial().add(LocalTime.now()+ " CONSULTA SALDO | SALDO ACTUAL: " + String.format("%.2f€", cuentaBank.getSaldo()));
